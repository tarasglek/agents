...
export class ChatModel {
  ...
  constructor(
    public chats: Chats,
    private allMessages: Store<Message>,
  ...
  async *llm(currentAgent: Agent, chatId?: string): AsyncGenerator<string> {
    ...
    msgsBeforeAI = msgsBeforeAI.filter((msg) =>
      !(this.options.USE_OPENROUTER && msg.type === "hosted_tool_call")
    );
  ...
  static async init(options: ModelOptions) {
    ...
    const messagesStore = new RelativeStore<Message>(
      diskStore as unknown as Store<Message>,
      "messages",
    );
