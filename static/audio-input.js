...
(async () => {
  try {
    ...
    log("Voice assistant initialized. Listening for wake word...");
    while (true) {
      log("Waiting for next command...");
      const audioUrl = await assistant.getNextCommand();
      if (audioUrl) {
        log(`Got command, playing audio from ${audioUrl}`);
